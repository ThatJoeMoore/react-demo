...
function searchProducts(req, res, ctx) {
  ...
  if (!sorter) {
    return res(ctx.status(400), ctx.json({ error: "Not a valid sort field" }));
  }
...
function textFilter(filter) {
  ...
  return ({ name, short_desc, long_desc }) =>
    ...
    long_desc.some((l) => l.toLowerCase().includes(lowerFilter));
}
...
function fetchProduct(req, res, ctx) {
  ...
  return res(ctx.status(product ? 200 : 404), ctx.json(product || {}));
}
